...
async function render() {
  ...
  if (paused) [state.textContent, state.className] = ["Paused", "badge paused"];
    ...
    [state.textContent, state.className] = ["Tracking", "badge tracking"];
  else if (st.idleState === "idle" || st.idleState === "locked")
    [state.textContent, state.className] = ["Idle", "badge"];
  else [state.textContent, state.className] = ["Not tracking", "badge"];

  $("domain").textContent = cur
    ? cur.domain + cur.url_path
    : "No active session";
  $("title").textContent = cur?.title ?? "";
  $("elapsed").textContent = cur
    ? formatDuration(Date.now() - cur.start_ms)
    : "";

  $("queued").textContent = (st.queue ?? []).length;
  const fs = st.flushStatus;
  $("sync").textContent = fs?.at
    ? `${fs.ok ? "ok" : "failed"} · ${new Date(fs.at).toLocaleTimeString()}`
    : "never";
...
$("pause").addEventListener("click", () =>
  chrome.runtime.sendMessage({ type: "setPaused", paused: !paused }),
);
